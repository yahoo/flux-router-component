...
describe('History', function () {
    ...
    describe('constructor', function () {
        ...
        it ('useHashRoute=true; has pushState', function () {
            var history = new History({win: windowMock.HTML5, useHashRoute: true});
            expect(history.win).to.equal(windowMock.HTML5);
            expect(history._hasPushState).to.equal(true);
            expect(history._useHashRoute).to.equal(true);
        });
        it ('useHashRoute=false; no pushState', function () {
            var history = new History({win: windowMock.OLD, useHashRoute: false});
            expect(history.win).to.equal(windowMock.OLD);
            expect(history._hasPushState).to.equal(false);
            expect(history._useHashRoute).to.equal(false);
        });
        it ('useHashRoute is a function', function () {
            var history = new History({win: windowMock.HTML5, useHashRoute: function (win) {
                return true;
            }});
            expect(history.win).to.equal(windowMock.HTML5);
            expect(history._hasPushState).to.equal(true);
            ...
            history = new History({win: windowMock.HTML5, useHashRoute: function (win) {
                return false;
            }});
            expect(history.win).to.equal(windowMock.HTML5);
            expect(history._hasPushState).to.equal(true);
    ...
    describe('getHash', function () {
        ...
        it ('non-empty hash with transformer', function () {
            var win = _.extend({}, windowMock.HTML5, {
                location: {
                    pathname: '/path/to/page',
                    hash: '#-path-to-abc'
    ...
    describe('getPath', function () {
        ...
        it ('has pushState with query', function () {
            ...
            var history = new History({win: win});
            var path = history.getPath();
            expect(path).to.equal('/path/to/page?foo=bar&x=y');
        });
        it ('no pushState', function () {
            ...
            history = new History({win: win});
            path = history.getPath();
            expect(path).to.equal('/path/to/page', 'hash=#');
        ...
        it ('no pushState, with query', function () {
            ...
            history = new History({win: win});
            path = history.getPath();
            expect(path).to.equal('/path/to/abc?foo=bar&x=y');
            ...
            history = new History({win: win});
            path = history.getPath();
            expect(path).to.equal('/?foo=bar&x=y');
        });
    });
    ...
    describe('pushState', function () {
        ...
        it ('useHashRoute=true; has pushState', function () {
            ...
            var history = new History({win: win, useHashRoute: true});
